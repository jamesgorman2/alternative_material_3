...
class _RunMetrics {
  ...
  @override
  String toString() {
    return '_RunMetrics(mainAxisExtent: $mainAxisExtent, crossAxisExtent: $crossAxisExtent, childCount: $childCount)';
  }
}

/// Parent data for use with [RenderWrapWithTrailing].
class WrapWithTrailingParentData extends ContainerBoxParentData<RenderBox> {
  int _runIndex = 0;
}

/// Displays its children in multiple horizontal or vertical runs.
///
/// A [RenderWrapWithTrailing] lays out each child and attempts to place the child adjacent
...
/// [RenderWrapWithTrailing] creates a new _run_ adjacent to the existing children in the
/// cross axis.
///
...
class RenderWrapWithTrailing extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, WrapWithTrailingParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, WrapWithTrailingParentData> {
  /// Creates a wrap render object.
  ///
  /// By default, the wrap layout is horizontal and both the children and the
  /// runs are aligned to the start.
  RenderWrapWithTrailing({
  ...
  /// grouped together in the center of the overall [RenderWrapWithTrailing] in the cross
  /// axis.
  ///
  /// Defaults to [WrapAlignment.start].
  ///
  ...
  ///
  /// If there is additional free space in the overall [RenderWrapWithTrailing] (e.g.,
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! WrapWithTrailingParentData) {
      child.parentData = WrapWithTrailingParentData();
    }
  }
    ...
    while (child != null) {
      if (child == lastChild) {
        ...
        final double remainingSize =
            mainAxisLimit - runMainAxisExtent - spacing * 2;
        switch (direction) {
          case Axis.horizontal:
            trailingConstraints = BoxConstraints(
              minWidth: remainingSize,
              maxWidth: constraints.maxWidth,
            );
          case Axis.vertical:
            trailingConstraints = BoxConstraints(
              minHeight: remainingSize,
              maxHeight: constraints.maxHeight,
            );
        }
        child.layout(trailingConstraints, parentUsesSize: true);
      } else {
      ...
      runCrossAxisExtent = math.max(runCrossAxisExtent, childCrossAxisExtent);
      childCount += 1;
      final WrapWithTrailingParentData childParentData =
          child.parentData! as WrapWithTrailingParentData;
